...
/**
 * 修改Hibernate源码：自动创建类的时候保持属性-字段的顺序
 * @author Lee[lee@leesoft.cn]
 * @datetime 2017年11月29日 下午9:57:33
 */
class PropertyContainer {
    ...
    /**
     * Constains the properties which must be returned in case the class is accessed via {@code AccessType.FIELD}. Note, this does not mean that all
     * {@code XProperty}s in this map are fields. Due to JPA access rules single properties can have different access type than the overall class
     * access type.
     */
    private final LinkedHashMap<String, XProperty> fieldAccessMap;

    /**
     * Constains the properties which must be returned in case the class is accessed via {@code AccessType.Property}. Note, this does not mean that
     * all {@code XProperty}s in this map are properties/methods. Due to JPA access rules single properties can have different access type than the
     * overall class access type.
     */
    ...
    /**
     * Retrieves all properties from the {@code xClass} with the specified access type. This method does not take any jpa access rules/annotations
     * into account yet.
    ...
    private static boolean discoverTypeWithoutReflection(XProperty p) {
        if (p.isAnnotationPresent(OneToOne.class) && !p.getAnnotation(OneToOne.class).targetEntity().equals(void.class)) {
            return true;
        } else if (p.isAnnotationPresent(OneToMany.class) && !p.getAnnotation(OneToMany.class).targetEntity().equals(void.class)) {
            return true;
        } else if (p.isAnnotationPresent(ManyToOne.class) && !p.getAnnotation(ManyToOne.class).targetEntity().equals(void.class)) {
            return true;
        } else if (p.isAnnotationPresent(ManyToMany.class) && !p.getAnnotation(ManyToMany.class).targetEntity().equals(void.class)) {
            return true;
