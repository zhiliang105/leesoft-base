...
function addTab(idSuffix,url,text,iconClass) {
	...
	if($("#" + tabId).length > 0) { // 选项卡已经存在
		refreshCurrentTab(idSuffix, false);
	} else {
		pageLoading();
		...
		$("<div class='tab-pane fade' id='"+tabId+"'><iframe id='"+frameId+"' frameborder='' src='"+url+"'></iframe></div>").appendTo(content);
...
function refreshCurrentTab(idSuffix, isLoc) {
	var liObj = null;
	var frame = null;
