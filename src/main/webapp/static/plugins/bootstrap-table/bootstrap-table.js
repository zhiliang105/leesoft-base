...
(function ($) {
    ...
    BootstrapTable.DEFAULTS = {
        ...
        showRefresh: false,
        showQuery : false, // TODO 自定义按钮,是否显示查询条件显示/隐藏按钮
        showToggle: false,
    ...
    BootstrapTable.prototype.initToolbar = function () {
        ...
        // TODO 自定义属性：是否显示查询条件显示和隐藏按钮；该按钮只定义是否显示,不添加按钮事件，按钮事件单独处理,通过class='btn-cus-e-bind'额外绑定事件
        if (this.options.showQuery) {
            html.push(sprintf('<button class="btn-cus-e-bind btn' +
                    sprintf(' btn-%s', this.options.buttonsClass) +
                    sprintf(' btn-%s', this.options.iconSize) +
                    '" type="button" name="query" aria-label="query" title="%s">',
                    "显示/隐藏查询条件"),
                sprintf('<i class="%s %s"></i>', "glyphicon", "glyphicon-search"),
                '</button>');
        }
